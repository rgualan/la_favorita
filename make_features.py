...
def read_cached(name) -> pd.DataFrame:
    ...
    cached = 'data/{}.pkl'.format(name)
    sources = ['data/{}.csv'.format(name), 'data/{}.csv.zip'.format(name)]
...
def read_all() -> pd.DataFrame:
    ...
    if os.path.exists(path):
        ...
        # Official data
        df = read_file('train2017')

        # Index
        # Fill gaps in dates. Improved with the suggestion from Paulo Pinto
...
def find_start_end(data: np.ndarray):
    """
    Calculates start and end of real traffic data. 
    Start is an index of first non-zero, non-NaN value,
...
def prepare_data(start, end, valid_threshold) -> Tuple[pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray]:
    """
    Reads source data, calculates start and end of each series, drops bad series, 
    calculates log1p(series)
    ...
    :param valid_threshold: minimal ratio of series real length to entire (end-start) 
    interval. Series dropped if ratio is less than threshold
